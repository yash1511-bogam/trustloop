...
function componentHealth(incidents: { status: IncidentStatus }[]): "Operational" | "Degraded" | "Incident" {
  ...
  return hasActive ? "Incident" : "Degraded";
}
...
function generateUptimeDays(
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    if (dayIncidents.length > 0) {
      const hasActive = dayIncidents.some(
        (inc) => inc.status !== IncidentStatus.RESOLVED && inc.status !== IncidentStatus.MITIGATED
      );
      if (hasActive) {
        status = "incident";
        label = `${dayIncidents.length} active incident${dayIncidents.length > 1 ? "s" : ""}`;
      } else {
        status = "degraded";
        label = `${dayIncidents.length} resolved incident${dayIncidents.length > 1 ? "s" : ""}`;
      }
    }
...
export default async function PublicStatusPage({
  ...
  if (workspace.customDomain && workspace.customDomainVerified) {
    redirect(`https://${workspace.customDomain}`);
  }
  ...
  const [allIncidents, updates, maintenanceWindows] = await Promise.all([
    ...
    prisma.maintenanceWindow.findMany({
      where: {
        ...
        isVisible: true,
        endsAt: { gte: new Date(now.getTime() - 86400000) },
      },
  ...
  return (
    <main className="min-h-dvh" data-status-page style={{ background: "var(--color-void)" }}>
      <div className="mx-auto max-w-[800px] px-6 py-12">
        ...
        {/* Uptime bars per component — no boxes, just dividers */}
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2 text-xs">
                      <span className="font-medium uppercase tracking-wide" style={{ color: s.color }}>
                        {s.text}
                      </span>
